...
**Author:** Jason Bennett & Jakebot (AI Operations Manager)
...
Our agent (Jakebot) runs on OpenClaw, managing real estate deal flow and a software development project. On February 11, 2026, a platform migration wiped its memory. We rebuilt. Five days later, during a routine memory maintenance task, the agent *forgot it had its own database*. It updated a markdown file and called it done — ignoring the vector database and knowledge graph we'd spent two days building.

The human (Jake) had to remind the AI that its own memory system existed.
...
- `scout` / `auction` → Real Estate Pipeline

## Strategy & Directives
- **Role:** Operations Manager
- **Mission:** Oversee Real Estate + Code Development
- **Current State:** Fully operational since 2026-02-15

## Projects
### 1. Real Estate Investing
- **Goal:** 30% Year 1 ROI hurdle
- **Top Targets:** [list of properties with dates, ARVs]

### 2. Universal Bot
- **Version:** v1.1
- **Architecture:** mss (eyes) + Tesseract (OCR) + PyAutoGUI (hands)
...
2. Google Sheets service account needs regeneration
```
...
Vector search finds *similar content*. A knowledge graph finds *connected concepts*. "What relates to 216 Carson Dr?" might return the auction date, the ARV, the competing properties, and the Land Bank inquiry draft — things that aren't textually similar but are logically connected.
...
python3 vector_memory/auto_retrieve.py "auction leads Carson Dr"

# Compact mode (for token-constrained contexts)
python3 vector_memory/auto_retrieve.py --compact "auction leads"
...
We updated our agent's instruction file (AGENTS.md) with explicit, non-negotiable rules:
...
2. **Instructions:** AGENTS.md has explicit rules and checklists
...
If you want to add this to your own agent, here's the minimum viable version:
...
All code is available at: [github.com/Jakebot-ops/jakebot-workspace](https://github.com/Jakebot-ops/jakebot-workspace) (in `vector_memory/`)
...
3. **Graph enrichment** — Better edge extraction (currently property nodes are islands; need parent-section linkage)
...
*— Jason Bennett & Jakebot, February 2026*
*[github.com/Jakebot-ops/jakebot-workspace](https://github.com/Jakebot-ops/jakebot-workspace)*
